...
export async function action({ request }: Route.ActionArgs) {
  ...
  const { clientGroupID, cookie, lastMutationID, profileID, pullVersion, schemaVersion } = body;
  ...
  if (!clientID) {
    console.error("Missing clientID in request. Available fields:", Object.keys(body));
  ...
  console.log(`Processing pull for clientID: ${clientID}, cookie: ${cookie}, lastMutationID: ${lastMutationID}, pullVersion: ${pullVersion}`);
  ...
  const serverCookie = latestProduct ? latestProduct.updatedAt.getTime() : Date.now();
  ...
  if (isNewClient || isOutdated) {
    console.log(`Sending full sync to client ${clientID}. New client: ${isNewClient}, Outdated: ${isOutdated}`);
  ...
  console.log(`Sending response with ${patch.length} operations, cookie: ${serverCookie}`);
